---
tipo: aula
resumo: "Incubadoras e aceleradoras: conceitos, diferenças e papel no ecossistema empreendedor para apoiar startups e novos negócios."
tags:
  - aula
  - incubadora
  - aceleradora
...
```mermaid
flowchart LR
    A[💡 Ideia] --> B[Incubadora\n12-36 meses]
    B --> C[MVP Validado]
    C --> D[Aceleradora\n3-6 meses]
    D --> E[Demo Day\nApresentação\npara Investidores]
    E --> F{Resultado}
    F -->|Captação| G[Série A/B\nEscalonamento]
    F -->|Sem captação| H[Bootstrapping\nou nova rodada]
...
```mermaid
flowchart TD
    P[Problema Identificado] --> I[Incubadora\nvalidar a ideia]
    I --> M[MVP construído]
    M --> A[Aceleradora\ncrescer rápido]
    A --> DD[Demo Day]
    DD --> IN[Investidor Anjo\nou Fundo VC]
    IN --> S[Startup Escalável]

    I -->|"Sem tração\napós 2+ anos"| X[Pivô ou encerramento]
    A -->|"Sem investidor\nno Demo Day"| Y[Bootstrapping\ncontinuado]
