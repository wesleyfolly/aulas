---
tipo: aula
resumo: "Conceitos e práticas de branding: construção de identidade de marca, diferenciação no mercado e criação de valor percebido pelos clientes."
tags:
  - aula
  - branding
  - marca
...
```mermaid
graph TD
    A[🏷️ Marca] --> B[Identidade Visual]
    A --> C[Identidade Verbal]
    A --> D[Experiência]

    B --> B1[Logo / Símbolo]
    B --> B2[Paleta de Cores]
    B --> B3[Tipografia]
    B --> B4[Padrões e Ícones]

    C --> C1[Nome da Marca]
    C --> C2[Slogan / Tagline]
    C --> C3[Tom de Voz]
    C --> C4[Linguagem e Vocabulário]

    D --> D1[Atendimento ao Cliente]
    D --> D2[Embalagem / Interface]
    D --> D3[Redes Sociais]
    D --> D4[Pós-venda]
```
...
graph LR
    ...
    A --> C[Ritmo]
    A --> D[Vocabulário]
    A --> E[Valores transmitidos]

    B --> B1["Formal / Informal"]
    B --> B2["Sério / Descontraído"]
    B --> B3["Especialista / Acessível"]

    C --> C1["Frases curtas = urgência"]
    C --> C2["Frases longas = reflexão"]

    D --> D1["Técnico ou cotidiano?"]
    D --> D2["Gírias ou padrão culto?"]

    E --> E1["Empatia, autoridade, humor..."]
```
...
```mermaid
quadrantChart
    title Mapa de Posicionamento: Preço x Qualidade Percebida
    x-axis Preço Baixo --> Preço Alto
    y-axis Qualidade Baixa --> Qualidade Alta
    quadrant-1 Premium
    quadrant-2 Custo-Benefício
    quadrant-3 Economia
    quadrant-4 Cilada
    Apple: [0.85, 0.90]
    Samsung: [0.65, 0.80]
