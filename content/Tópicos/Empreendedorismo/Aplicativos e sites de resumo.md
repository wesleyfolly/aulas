...
```mermaid
flowchart TD
    A[Quero resumir um livro] --> B{Já tenho o PDF/texto?}
    B -- Sim --> C[Ferramenta de IA generativa]
    B -- Não --> D{Prefiro em português?}
    D -- Sim --> E[12min ou Headway]
    D -- Não/Tanto faz --> F{Quero profundidade ou velocidade?}
    F -- Profundidade --> G[Shortform]
    F -- Velocidade --> H[Blinkist ou Headway]
    C --> I{Grátis ou pago?}
    I -- Grátis --> J[ChatGPT Free ou Claude Free]
    I -- Pago --> K[ChatGPT Plus, Claude Pro ou BeFreed]
    E --> L[Testar versão gratuita primeiro!]
    G --> L
    H --> L
    J --> M[Checar fatos contra o livro original]
    K --> M
```
