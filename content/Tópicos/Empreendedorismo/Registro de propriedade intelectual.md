---
tipo: aula
resumo: "Registro de propriedade intelectual: processos e importância de proteger ideias, marcas, patentes e direitos autorais no contexto empreendedor."
tags:
  - aula
...
```mermaid
flowchart TD
    A([Início: tenho um nome/logo]) --> B[Busca de anterioridade\nbusca.inpi.gov.br]
    B --> C{Nome disponível\nna classe desejada?}
    C -- Não --> D[Revisar nome ou\nescolher outra classe]
    D --> B
    C -- Sim --> E[Definir natureza da marca\nMista, Nominativa, Figurativa, 3D]
    E --> F[Escolher classificação\nNice: qual das 45 classes?]
    F --> G[Depositar pedido\nservicos.inpi.gov.br]
    G --> H[Pagar taxa unificada\nex.: R$440 para MEI]
    H --> I[Publicação na RPI\nRevista da Propriedade Industrial]
    I --> J{Oposição de\nterceiros em 60 dias?}
    J -- Não --> K[Exame de mérito\npelo INPI]
    J -- Sim --> L[Responder à oposição]
    L --> K
    K --> M{Aprovado?}
    M -- Sim --> N([Registro concedido\nVigência: 10 anos])
    M -- Não --> O[Recurso ao INPI]
    O --> K
```
...
```mermaid
flowchart TD
    A([Tenho uma invenção]) --> B[Busca de anterioridade\nbase do INPI + Google Patents]
    B --> C{Já existe algo\nparecido?}
    C -- Sim --> D[Avaliar diferencial\noferecido pela invenção]
    D --> B
    C -- Não --> E[Preparar documentação técnica\nRelatório + Reivindicações + Desenhos]
    E --> F[Depósito via e-Patentes\nservicos.inpi.gov.br]
    F --> G[Pagar taxa de depósito\nex.: R$130 com desconto MEI]
    G --> H[Sigilo por 18 meses\ndepois publicado na RPI]
    H --> I[Solicitar exame\naté 36 meses do depósito]
    I --> J[Exame técnico pelo INPI\npode gerar exigências]
    J --> K{Aprovado?}
    K -- Sim --> L([Carta Patente expedida\nautomaticamente, sem custo])
    K -- Não --> M[Recurso / Novo pedido]
```
