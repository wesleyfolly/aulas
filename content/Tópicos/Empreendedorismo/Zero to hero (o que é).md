---
tipo: aula
resumo: "Zero to hero: conceito e metodologia de transformação pessoal e profissional, jornada de desenvolvimento de habilidades do iniciante ao especialista."
tags:
  - aula
  - zero-to-hero
  - desenvolvimento
...
```mermaid
timeline
    title Jornada Zero to Hero de um Empreendedor
    section Fase 0 - Ignição
        Identifica o problema : Percebe que algo não funciona bem
        Valida a dor : Conversa com 10 a 20 pessoas reais que sofrem o problema
    section Fase 1 - Arranque
        Lança o MVP : Versão mínima do produto (dias ou semanas, não meses)
        Consegue os 3 primeiros clientes : Vende manualmente, sem escalar
        Gera a primeira receita : Qualquer valor real já quebra o "zero"
    section Fase 2 - Tração
        Encontra o Product-Market Fit : Clientes retornam e indicam sem incentivo
        Monta o time : Contrata ou busca sócios para cobrir lacunas
        Capta os primeiros recursos : Aceleradoras, anjos, receita orgânica
    section Fase 3 - Escala
        Automatiza processos : Não depende mais do fundador em tudo
        Expande geograficamente ou de segmento : Vai além do mercado inicial
        Consolida posição : Referência no nicho, marca reconhecida
    section Fase 4 - Herói
        Influencia o ecossistema : Mentorias, investimentos, referência pública
        Cria novos ciclos : Novo projeto, fundo, comunidade ou empresa
```
...
```mermaid
flowchart TD
    A[🥚 ZERO\nIdeia + Problema] --> B[🔬 VALIDAÇÃO\nPesquisa + Entrevistas]
    B --> C[🛠️ MVP\nProtótipo mínimo]
    C --> D[💰 PRIMEIROS CLIENTES\nVenda manual]
    D --> E{Product-Market Fit?}
    E -->|Não| B
    E -->|Sim| F[📈 ESCALA\nTime + Processos + Capital]
    F --> G[🏆 HERÓI\nReferência + Impacto]
