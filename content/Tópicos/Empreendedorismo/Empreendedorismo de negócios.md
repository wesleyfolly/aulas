---
tipo: aula
resumo: "Empreendedorismo de negócios: conceitos e práticas para criação e gestão de empresas, incluindo plano de negócios e estratégias empresariais."
tags:
  - aula
...
```mermaid
graph LR
    A["🤝 Parcerias-Chave"] --> B["⚙️ Atividades-Chave"]
    B --> C["🎁 Proposta de Valor"]
    C --> D["💬 Relacionamento\ncom Clientes"]
    D --> E["👥 Segmento\nde Clientes"]
    F["🏗️ Recursos-Chave"] --> C
    C --> G["📢 Canais"]
    G --> E
    H["💸 Estrutura de Custos"] -.-> I["💰 Fontes de Receita"]
```
...
```mermaid
xychart-beta
    title "Abertura de pequenos negócios no Brasil (milhões)"
    x-axis ["2022", "2023", "2024", "2025", "2026*"]
    y-axis "Empresas abertas (mi)" 0 --> 6
    bar [3.2, 3.8, 4.3, 5.1, 2.0]
```
...
```mermaid
flowchart TD
    A["Quero abrir um negócio"] --> B{"Trabalharei sozinho?"}
    B -- Sim --> C{"Fatura até\nR$ 81.000/ano?"}
    B -- Não --> D["Sociedade Limitada\n(LTDA) ou S/A"]
