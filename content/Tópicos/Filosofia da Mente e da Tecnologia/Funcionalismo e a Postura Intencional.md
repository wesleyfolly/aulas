---
tipo: aula
resumo: "A resposta funcionalista a Searle: estados mentais definidos pelo que fazem, não pelo material (Putnam, realização múltipla). A postura intencional de Dennett (1987) e suas três posturas para prever sistemas. Faz sentido dizer que o ChatGPT 'quer' ou 'acha' algo?"
tags:
  - aula
  - filosofia-da-mente
  - filosofia-da-ia
...
```mermaid
flowchart TD
    S["Quero prever<br/>o comportamento de um sistema"] --> P1{"Sei a física<br/>dele toda?"}
    P1 -->|Sim, simples| F["🪨 Postura Física"]
    P1 -->|Não| P2{"Sei para que<br/>foi projetado?"}
    P2 -->|Sim| D["⏰ Postura de Projeto"]
    P2 -->|"Complexo demais"| I["🧠 Postura Intencional:<br/>trato como se tivesse<br/>crenças e desejos"]
```
...
mindmap
  root(("Mente é função?"))
    Funcionalismo
      Estado mental = papel causal
      Software roda em qualquer hardware
      Realização múltipla (Putnam)
    Postura Intencional
      Física
      De projeto
      Intencional (crenças e desejos)
    Dennett x Searle
      Compreensão é do sistema todo
      Simular = duplicar?
      Sem árbitro final
    Pra sua vida
      Antropomorfismo calibrado
      Pensar por função é engenharia
```
