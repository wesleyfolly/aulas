---
title: "Programação"
tipo: index
resumo: "Hub da disciplina de Programação, abrangendo fundamentos, linguagens, paradigmas e projetos práticos."
tags:
  - index
  - programacao
  - python
  - javascript
aliases:
  - Programação
---

# Programação

> [!quote] A Arte de Resolver Problemas
> *Este espaço é um guia usado em minhas aulas de **Programação**. O objetivo é listar alguns **tópicos** e não necessariamente prover todo o conteúdo necessário.*

---

## 📝 Avaliações

[[Trabalhos e Projetos de Programação]]

---

## 🔥 Linguagens Mais Usadas em 2025

> [!info] Top 5 Global

| Posição | Linguagem | Destaque |
|---------|-----------|----------|
| 1º | **Python** | IA, Data Science, Automação |
| 2º | **Java** | Enterprise, Android |
| 3º | **JavaScript** | Web, Full Stack |
| 4º | **C++** | Sistemas, Games |
| 5º | **Go** | Cloud, DevOps |

---

## 📚 Conteúdo Teórico

> [!tip] Fundamentos

- [[Introdução à programação com python]]
- [[Conceitos gerais de programação]]
- [[Paradigmas de programação]]
- [[Estrutura de dados]]

---

## 🛠️ Recursos e Ferramentas

> [!info] Ambiente de Desenvolvimento

- [[Recursos e Ferramentas]]
- [[Instalação e configurações]]
- [[GitHub e controle de versão]]
- [[Tópicos/Programação/Materiais|Materiais]]

---

## 🐍 Python

> [!success] Linguagem Principal do Curso

- [[Python]]
- [[Materiais e conceitos básicos]]
- [[Banco de códigos e Bibliotecas]]
- [[Armazenamento de senhas]]

### 📓 Notebooks

- [[Google colaboratory]]
- [[Jupyter Notebook]]
- [[JupyterLab]]

---

## 🌐 JavaScript

> [!info] Desenvolvimento Web

- [[JavaScript]]

---

## 🚀 Projetos Práticos

> [!success] Mãos na Massa

- [[Projeto Programador nômade minimalista]]
- [[Projeto Automação de vida com python]]

---

## 🔬 Pesquisa e Inovação

> [!tip] Tendências

- [[Novas tecnologias e pesquisa]]

